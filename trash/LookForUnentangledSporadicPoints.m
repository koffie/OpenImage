...
/*  We have loaded "gl2tab" from the computations of Sutherland and Zywina.
    It is an associative array of all subgroups G of GL_2(Z_p) for a prime p, up to conjugacy, that have following properties:
...
keys_to_consider:={};
keys:=[k: k in Keys(X) | X[k]`is_entangled eq false and X[k]`genus eq 0];
for k1 in keys do
for k2 in keys do
    N1:=X[k1]`N;
    N2:=X[k2]`N;
    if GCD(N1,N2) ne 1 or N1 ge N2 then continue k2; end if;
    k1;k2;

    k:=k1;
    for i in [1..#k] do
        if k2[i] ne "1A0-1a" then k[i]:=k2[i]; end if;
    end for;
    //G1:=gl2Lift(X[k1]`G,N1*N2);
    //G2:=gl2Lift(X[k1]`G,N1*N2);
    //G:=G1 meet G2;
    keys_to_consider:=keys_to_consider join {k};
end for;
end for;

keys_to_consider:=[k : k in keys_to_consider | k notin Keys(X) or X[k]`has_infinitely_many_points eq false];

function key_lt(k1,k2)
    flag:=true;
    for i in [1..#k1] do
        if k2[i] notin allsupeq[k1[i]] then
            flag:=false;
        end if;
    end for;
    if k1 eq k2 then flag:=false; end if;
    return flag;
end function;

keys_to_consider_min:={};
for k in keys_to_consider do
    if exists{k1: k1 in keys_to_consider | key_lt(k1,k)} eq false then
        keys_to_consider_min:=keys_to_consider_min join {k};
    end if;
end for;
#keys_to_consider_min;


k:=Random(keys_to_consider_min);
    ...
    repeat
        ...
        for a in S do
            ...
            if not is_serre_type then
                ...
                // These cases are the slowest using the other approach.
                k0:=a; k0[1]:="1A0-1a";   // X[a]-> X[k0] has degree 2
            ...
            if M`N eq 1 then            
                ...
                // TODO: added g, does it catch on...  REMOVE WHEN SURE
    ...
    for k in Keys(X) do
        if assigned X[k]`phiC then
            r:=Rank(Parent(X[k]`phiC[1]));
            Pol<[x]>:=PolynomialRing(Rationals(),r);
            X[k]`phiC:=[Pol!a: a in X[k]`phiC];
        end if;

        delete X[k]`map_to_jline;

        // we are deleting these next two just to keep file sizes down (OK?)
        delete X[k]`F;
        delete X[k]`F0; 
    end for;
